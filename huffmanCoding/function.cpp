...
string decodeTextFromBit(const string& encodedBits, unordered_map<string, char>& reverseHuffmanTable) {
	string decodedText = "";
	string currentCode = "";
	for (char bit : encodedBits) {
		currentCode += bit;
		if (reverseHuffmanTable.find(currentCode) != reverseHuffmanTable.end()) {
			decodedText += reverseHuffmanTable[currentCode]; //Tìm thấy mã hợp lệ -> thêm vào kết quả
			currentCode = "";
		}
	}
	return decodedText;
